...
export const ROLE_PERMISSIONS: Record<SystemRole, AdminPermission[]> = {
  super_admin: [
    "users.manage",
    "users.ban",
    "users.delete",
...
export interface SystemSettings {
  ...
  captchaFrontendUrl: string; // 基础URL，前端自行拼接 /cap.min.js 和 /api/
  ...
  aiAvatarUrl: string;
  // CDN配置
  cdnFontsGoogleCssUrl: string; // Google Fonts CSS URL，留空则不加载
  cdnVconsoleEnabled: boolean; // 是否启用 VConsole 调试工具
  // ICE服务器配置
  iceStunUrls: string; // JSON格式的STUN服务器列表
  iceTurnUrl: string; // JSON格式的TURN服务器列表
  iceTurnUsername: string; // 保留向后兼容（空）
  iceTurnCredential: string; // 保留向后兼容（空）
  // 翻译相关
  ...
  smsSdkAppId: string;        // 腾讯云 SMS 应用 ID
  smsAccessKeyId: string;     // 阿里云 / 腾讯云 SecretId
  smsAccessKeySecret: string; // 阿里云 AccessKeySecret / 腾讯云 SecretKey
  smsSignName: string;        // 短信签名
  smsTemplateCode: string;    // 短信模板 ID（阿里云 TemplateCode / 腾讯云 TemplateId）
  smsRegion: string;          // 阿里云地域（如 cn-hangzhou），腾讯云可不填
  smsEndpoint: string;        // 短信 API 接入点（可选，留空使用默认）
}
...
export interface ConversationMember {
  userId: ID;
  role: ChannelRole;
  muted: boolean; // individually muted by admins
  banned: boolean;
  joinedAt: ISODate;
}
...
export interface Conversation {
  ...
  isPrivate?: boolean;
  icon?: string; // emoji
  avatarUrl?: string; // channel uploaded avatar
  muteAll?: boolean; // when true, only owner/admins can send
  membersCanInvite?: boolean; // when false, only owner/admins can add members
  /** Membership */
  memberIds: ID[];
  members?: ConversationMember[]; // detailed roster (channels)
  ownerId?: ID;
  createdAt: ISODate;
  /** Denormalized for list rendering */
  lastMessageId?: ID;
  lastMessageAt?: ISODate;
  pinned?: { messageId: ID; pinnedBy: ID; pinnedAt: ISODate }[];
}
...
export interface PollResult {
  ...
  text: string;
  count: number;
  voters: { userId: ID; displayName: string }[];
}
...
export interface BootstrapData {
  ...
  channelReadStates: Record<ID, Record<ID, { lastReadAt: ISODate; lastReadMessageId: ID }>>;
...
// ----------------------------------------------------------------------------
// WebSocket protocol — discriminated unions on `type`
// ----------------------------------------------------------------------------

/** Client -> Server */
export type ClientEvent =
  | { type: "auth"; token: string }
  | { type: "message:send"; payload: SendMessageRequest; clientId: string }
  | { type: "typing:start"; conversationId: ID }
  | { type: "typing:stop"; conversationId: ID }
  | { type: "presence:set"; status: PresenceStatus }
  | { type: "reaction:toggle"; messageId: ID; emoji: string }
  | { type: "read"; conversationId: ID; messageId: ID }
  | { type: "message:recall"; messageId: ID }
  | { type: "message:edit"; messageId: ID; text: string }
  // Voice / video calls — the server is a pure SFU + signaling relay. The
  // browser handles the actual WebRTC state; the server forwards SDP/ICE
  // and acts as a Selective Forwarding Unit for multi-party media.
  | { type: "call:invite"; callId: ID; conversationId: ID; kind: CallKind }
  | { type: "call:accept"; callId: ID }
  | { type: "call:reject"; callId: ID }
  | { type: "call:cancel"; callId: ID }
  | { type: "call:hangup"; callId: ID }
  | { type: "call:offer"; callId: ID; sdp: string }
  | { type: "call:answer"; callId: ID; subscriberId: ID; publisherId: ID; sdp: string }
  | { type: "call:ice"; callId: ID; candidate: RTCIceCandidateInit; target?: "upstream" | "downstream"; subscriberId?: ID; publisherId?: ID }
  | { type: "call:subscribe"; callId: ID; publisherId: ID; kind: CallTrackKind }
  | { type: "call:admin"; callId: ID; action: "mute" | "unmute" | "ban"; userId: ID }
  | { type: "call:query-active" }
  | { type: "poll:vote"; messageId: ID; optionId: string }
  | { type: "presence:ping"; conversationId: ID }
  | { type: "presence:pong"; conversationId: ID; pingId: ID; toUserId: ID };

/** Server -> Client */
export type ServerEvent =
  | { type: "ready"; data: BootstrapData }
  | { type: "error"; message: string; code?: string; clientId?: string; conversationId?: ID }
  | { type: "captcha_required"; message: string; clientId?: string; conversationId?: ID }
  | { type: "message:new"; message: Message; clientId?: string }
  | { type: "message:update"; message: Message }
  | { type: "message:scheduled"; clientId: string; messageId: ID; scheduledAt: ISODate }
  | { type: "conversation:new"; conversation: Conversation }
  | { type: "conversation:update"; conversation: Conversation }
  | { type: "conversation:remove"; conversationId: ID }
  | { type: "typing"; conversationId: ID; userId: ID; isTyping: boolean }
  | { type: "presence"; userId: ID; status: PresenceStatus; lastSeen: ISODate }
  | { type: "read"; conversationId: ID; userId: ID; messageId: ID }
  | { type: "user:update"; user: PublicUser }
  | { type: "friend:request"; request: FriendRequest; from: PublicUser }
  | { type: "friend:update"; friendship: Friendship; user: PublicUser }
  | { type: "friend:remove"; userId: ID }
  | { type: "history:cleared"; conversationId: ID }
  // Call signaling — the server is the rendezvous/SDP/ICE relay AND the
  // SFU that re-broadcasts media tracks between participants.
  | { type: "call:incoming"; call: Call }
  | { type: "call:accepted"; callId: ID; byUserId: ID }
  | { type: "call:rejected"; callId: ID; byUserId: ID }
  | { type: "call:cancelled"; callId: ID }
  | { type: "call:hangup"; callId: ID; byUserId: ID }
  | { type: "call:answer"; callId: ID; fromUserId: ID; sdp: string }
  | { type: "call:downstream-offer"; callId: ID; subscriberId: ID; publisherId: ID; kind: CallTrackKind; sdp: string }
  | { type: "call:ice"; callId: ID; fromUserId: ID; candidate: RTCIceCandidateInit; target?: "upstream" | "downstream"; subscriberId?: ID; publisherId?: ID; kind?: CallTrackKind }
  | { type: "call:peer-joined"; callId: ID; userId: ID; kind: CallKind; publishing: CallTrackKind | null }
  | { type: "call:peer-left"; callId: ID; userId: ID }
  | { type: "call:track-published"; callId: ID; userId: ID; kind: CallTrackKind }
  | { type: "call:track-unpublished"; callId: ID; userId: ID; kind: CallTrackKind }
  | { type: "call:admin-event"; callId: ID; action: "mute" | "unmute" | "ban"; userId: ID; byUserId: ID }
  | { type: "call:banned"; callId: ID; userId: ID }
  | { type: "user:banned"; reason?: string }
  | { type: "notification:new"; notification: Notification }
  | { type: "notification:update"; notification: Notification }
  | { type: "notification:remove"; notificationId: ID }
  | { type: "call:active-calls"; calls: ActiveCallInfo[] }
  | { type: "poll:update"; messageId: ID; conversationId: ID; results: PollResult[]; totalVotes: number }
  | { type: "e2ee:started"; conversationId: ID; peerId: ID; sessionId?: string }
  | { type: "e2ee:ended"; conversationId: ID; peerId: ID; reason?: string }
  | { type: "presence:ping"; conversationId: ID; fromUserId: ID; fromName: string; pingId: ID }
  | { type: "presence:pong"; conversationId: ID; fromUserId: ID; pingId: ID };
